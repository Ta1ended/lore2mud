...
## 工作方式
- Codex 是唯一活动开发 Agent，负责方案、实现、测试、交接和本地提交；Hermes
  不再承担新任务，历史提交与决定中的 Hermes 归属保持不变。
...
- 本地提交不自动 push。
- 持续 Goal 中若已确认 GitHub `main` 落后本地超过 5 个提交，立即停止新实施与复验，
  通知项目负责人使用 GitHub Desktop push；远端同步并重新核实前不继续扩大差距。
