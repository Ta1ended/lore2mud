...
## 工作方式
- GPT-5.6-sol 是顾问：先审查范围、数据契约、架构边界、风险，并进行独立验收。
- Codex 是唯一执行者：只在顾问确认后的单一纵向切片内实现、测试、更新交接文件
  和创建本地提交。
- 项目负责人负责在 GPT-5.6-sol 与 Codex 两个对话之间人工转交顾问 prompt 和执行报告。
- 完成实施和本地验证不等于通过独立验收；必须经 GPT-5.6-sol 复核。
- 每次只能实施一个经顾问批准的纵向切片。
...
## 跨会话协作
- 每次方案、审核意见和完成报告必须以一个 `text` fenced code block 结束。
- 该代码块必须自包含，便于直接转交另一会话，无需翻找历史消息。
- 代码块内容：基线提交（hash 或 HEAD）、唯一任务、文件范围、风险、验证结果、
  下一动作。
- 项目负责人将该自包含代码块人工转交给另一角色；Codex 不替代顾问做独立验收。
