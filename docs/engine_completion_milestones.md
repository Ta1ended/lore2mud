...
- **状态**: M7 进行中。M7.1 已由 GPT-5.6-sol 独立验收 GO、无 findings；M7.2 已由 Codex
  本地实现并验证，等待 GPT-5.6-sol 独立验收。M7 的规模条件已本地满足，但不得表述为 M7 GO。
...
- **强制完成条件** — original_demo 至少达到：
  ...
  - 至少一个可实际买卖的商店。
  - 至少一个可复现的死亡/恢复流程。
- M7.2 已本地达到 M7 的内容规模条件；唯一下一动作是 GPT-5.6-sol 对 M7.2 的内容边界、
  规模、任务结算、0.8.0/v7 兼容和 CLI/save 证据独立验收。验收前不得宣布 M7 GO、开始 M8 或 push。
