...
_Last updated: 2026-07-30（M7.2/M7 GO；M8 只读审计基线完成，独立复核待定）_
...
- **状态**: GPT-5.6-sol 已于 2026-07-30 独立验收 M7.2 与整体 M7 为 GO、无 findings
  （DEC-0034）。这只封板 M7 原创内容规模，不等同于公共引擎完成；M8 已进入只读审计，独立复核待定。
...
- **强制完成条件** — original_demo 至少达到：
  ...
  - 至少一个可实际买卖的商店。
  - 至少一个可复现的死亡/恢复流程。
- M7 的全部强制条件已由原创演示满足并经独立验收封板。项目负责人随后确认 push 完成，
  Codex 已进入 M8 只读审计；M8 的独立复核仍是公共引擎完成的必要门槛。

## M8: 公共引擎完成独立审计

- **状态**: 只读审计基线已完成，等待 GPT-5.6-sol 独立复核（DEC-0035）；M8 尚未 GO。
- **审计基线**: `f486e12` 已同步到 `main`、`origin/main` 与直接查询的远端 `main`，工作树
  干净，ahead/behind 为 0/0。599 项全量 unittest、compileall、original-demo 内容校验、
  `check_repo_safety.py --history`、`git diff --check` 和 `git fsck --full --no-dangling` 全部通过。
  真实 CLI 完成 M7.2 两条分支与 v7/0.8.0 save/load；全新 CLI 进程证明棱镜哨卫击杀玩家后，
  `recover` 回到余烬渡台并完成 v7 save/load。
- **审计边界**: 本轮只读核对，未修改引擎、Schema、依赖、原创内容或 save 契约；M7 GO 仍不等同
  公共引擎完成，私有小说事实层不在 M8 范围内。
...
- 在 GPT-5.6-sol 独立复核完成前，只能称为"公共引擎开发中"或 `Conditional GO`。
