...
_Last updated: 2026-07-30（M8 GO；M1–M8 范围内公共引擎完成，等待后续明确授权）_
...
- **状态**: GPT-5.6-sol 已于 2026-07-30 独立验收 M7.2 与整体 M7 为 GO、无 findings
  （DEC-0034）。M7 只封板原创内容规模；随后 M8 已独立验收 GO（DEC-0036），完成 M1–M8
  范围内的公共引擎路线，不延伸为 M9 或私有事实层授权。
- **当前进度**: original_demo 已有 8/8 个房间、4/4 只怪物和 7 条任务；三类既有任务、
  对话 effects、堆叠物品、商店和死亡/恢复流程均仍在公共演示中。
- **M7.1 历史实现**: 静默观测站 east ↔ 碎讯支线 west；火花巡兽位于碎讯支线；进入观测站后
  接取 `quest_clear_spark_hound`，击败它复用现有 `monster_defeated` 结算。M7.1 当时内容包为
  0.7.0，save 为 v7，旧 0.6.0 内容包存档被版本检查拒绝。
- **M7.2 本地实现**: 碎讯支线 east ↔ 断轨岔口 west；岔口 north ↔ 雾凝机井 south，east ↔
  折光档案室 west，档案室 east ↔ 余辉信标台 west。雾核潜行者和棱镜哨卫分别位于两条新分支，
  在断轨岔口接取各自唯一目标的既有 `monster_defeated` 任务。内容包升至 0.8.0，save 保持 v7，
  旧 0.7.0 内容包存档被既有版本检查拒绝。
- **Codex 本地验证证据（M7.2）**: 8 项 M7 场景、15 项 loot 回归、599 项全量 unittest、
  compileall、original_demo 校验、历史安全扫描、diff 检查和仓库外 CLI/save v7 通过。
- **GPT-5.6-sol 独立验收（M7.2 与 M7，2026-07-30）**: 相对 `5497859` 的 `147633e` 核对
  1 个提交、22 个内容、测试和公开文档文件；`src/`、Schema、依赖文件与私有资料路径均为 0。
  23 项 M7/loot 聚焦和 599 项全量 unittest、图与唯一性审计、compileall、original_demo 校验、
  历史安全扫描和 `git diff --check` 均通过。真实 CLI/save 与死亡恢复均通过；存档为 v7/0.8.0，
  玩家在新增房间，两只新增怪物 HP=0 且已移除，两条新增任务完成。验收时直查远端 `main` 为
  `f0acd3f`，未 push。
- **GPT-5.6-sol 独立验收（2026-07-30）**: 相对 `086cda8` 核对 `9786325` 的 1 个提交、
  22 个文件、+333/-55；`src/`、`schemas/`、依赖文件与私有资料路径均为 0。19 项 M7/loot
  聚焦、595 项全量 unittest、compileall、original_demo 校验、历史安全扫描和基线范围
  `git diff --check` 均通过。真实 CLI 完成接取新任务、击败两只怪物、进入碎讯支线、
  完成 `quest_clear_spark_hound` 与 save/load；存档仍为 v7/0.7.0，玩家位于新房间，
  火花巡兽 HP 为 0 且已移除，任务完成。直查远端 `main` 仍为 `f0acd3f`，未 push。
- **强制完成条件** — original_demo 至少达到：
  ...
  - 至少一个可实际买卖的商店。
  - 至少一个可复现的死亡/恢复流程。
- M7 的全部强制条件已由原创演示满足并经独立验收封板。M8 的独立复核随后通过，成为
  M1–M8 公共引擎完成结论的最终门槛；该结论不自动开启后续路线图。

## M8: 公共引擎完成独立审计

- **状态**: GPT-5.6-sol 已于 2026-07-30 对 M8 聚焦复核给出 GO；Git 快照 P2 已关闭、无新增
  findings（DEC-0036）。M1–M8 范围内的公共引擎完成；这不是 M9、新功能或私有事实层的授权。
- **审计与 Git 记录**: 技术审计基线为 `f486e12`，审计记录提交为 `6510e2d`，P2 修正提交为
  `6502a72`。验收时本地 `HEAD` 与 `origin/main` 均为 `6502a72`，工作树干净、ahead/behind 为
  0/0；GitHub Desktop push 已反映到本地跟踪分支。命令行直接查询远端本轮超时，因此后续发布前
  仍须实时复查远端。
- **审计证据**: 599 项全量 unittest、compileall、original-demo 内容校验、
  `check_repo_safety.py --history`、`git diff --check` 和 `git fsck --full --no-dangling` 全部通过。
  既有 216 项专项、375 项 save 矩阵、35 项内容/CLI/安全矩阵和真实 CLI 证据仍有效；真实流程覆盖
  M7.2 两条分支、v7/0.8.0 save/load，以及棱镜哨卫击杀后的 `recover` 与存档恢复。
- **审计边界**: M8 只核对公共引擎路线，未改变引擎、Schema、依赖、原创内容或 save 契约。
  私有小说事实层不在 M1–M8 范围内；任何后续开发均须项目负责人另行明确授权。
...
- 上述完成条件已通过 GPT-5.6-sol 独立复核。M1–M8 范围内可称为“公共引擎完成”；该措辞不
  自动允许 M9、新功能、发布或私有事实层工作。
