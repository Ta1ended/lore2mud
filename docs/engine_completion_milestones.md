...
_Last updated: 2026-07-30（M7.1 已由 GPT-5.6-sol 独立验收 GO；M7 进行中）_
...
- **状态**: M7 进行中。M7.1 第二个原创遭遇已由 GPT-5.6-sol 独立验收 GO、无 findings；
  该结论只封板 M7.1，不得表述为 M7 GO。
- **当前进度**: original_demo 已有 4/8 个房间、2/4 只怪物和 5 条任务；三类既有任务、
  对话 effects、堆叠物品、商店和死亡/恢复流程均仍在公共演示中。M7 其余内容规模目标未完成。
- **M7.1 已实现**: 静默观测站 east ↔ 碎讯支线 west；火花巡兽位于碎讯支线；进入观测站后
  接取 `quest_clear_spark_hound`，击败它复用现有 `monster_defeated` 结算。内容包升至 0.7.0，
  save 仍为 v7，旧 0.6.0 内容包存档继续被版本检查拒绝。
- **Codex 本地验证证据**: 4 项 M7 场景、15 项 loot 回归、595 项全量 unittest、compileall、
  original_demo 校验、历史安全扫描、diff 检查和仓库外 CLI/save v7 通过。
...
- **强制完成条件** — original_demo 至少达到：
  ...
  - 至少一个可实际买卖的商店。
  - 至少一个可复现的死亡/恢复流程。
- 内容扩容队列第一步 M7.1 已独立验收 GO；M7 仍未达到规模完成条件。任何下一项 M7
  扩容仍须项目负责人单独授权；获得授权后必须先提交只读实施方案，不得直接开始 M7.2 或 push。
