...
_Last updated: 2026-08-01（NarrativeModel v1 P2 修复已本地验证；重新独立验收待办）_
...
`git rev-parse HEAD` 记录最终候选提交。不得以实现任务自己的本地测试替代独立 `GO`。
...
- 明确复核所有 scope claim 带理由省略时允许空 `claim_uses`，但缺失、foreign 或未记账 claim
  仍被拒绝，且两份 Schema 与格式文档一致。
- 复核更新后的 42 项专门测试（2 Windows symlink 权限跳过）、1298 项 full unittest 和
  1289 项 pytest（各 9 skips）、Ruff、Pyright、compileall、original demo 校验、history safety、
  fsck、diff 检查和仓库外标准 golden/全省略 CLI 路径。
- 复核 `PROJECT_MEMORY.md`、`PROJECT_STATE.md`、`NEXT_TASK.md`、`CHANGELOG.md` 和
  `DECISIONS.md` 的描述仍为“独立验收 pending”，不提前宣称 GO。

## Boundaries

- 本次验收只覆盖 NarrativeModel v1 的 P2 修复；不修改 GEN-1 或其他责任域。
